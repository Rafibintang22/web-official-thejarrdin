  ...
  static async readAllNotifPesan(penerima_id) {
    try {
      ...
      const transformedData = findMessage.map((msg) => ({
        Id: msg.pesan_id,
        FiturID: 8, //no fitur_id untuk masukan & aspirasi
      ...
      if (tipeRead === "untukUser") {
        findMessage = await PesanModel.findOne({
          ...
          include: [
            ...
            { model: PesanTujuanModel, where: { penerima_id: pengguna_id }, required: true },
          ],
        });
        ...
        const transformedData = {
          ...
          DibuatOleh: { UserID: findMessage.Pengguna.pengguna_id, Nama: findMessage.Pengguna.nama },
  ...
  static async create(dataInsert, isReply) {
    ...
    try {
      ...
      const newMessage = await PesanModel.create(
        {
          fitur_id: 8, //8 adalah ID Fitur Aspirasi
      ...
      const filteredUserTujuan = AllPengurusID.filter((user) => user.pengguna_id !== UserID_dibuat);
